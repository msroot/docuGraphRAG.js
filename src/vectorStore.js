...
export class Neo4jVectorStore {
    ...
    async initialize() {
        ...
        try {
            ...
            try {
                ...
                // If vector index creation fails (older Neo4j version), log the error but continue
    ...
    async addToStore(chunks, metadata, extractedInfo) {
        ...
        try {
            ...
            await session.run(
                'CREATE (d:Document) SET d = $properties',
                {
                    properties: {
                        id: documentId,
            ...
            for (let i = 0; i < chunks.length; i++) {
                ...
                // Create DocumentChunk node with embedding and connect to Document
                await session.run(
                    ...
                    'CREATE (d)-[:HAS_CHUNK]->(c)',
                    {
                        properties: {
                            ...
                            chunkIndex: i,
                            embedding: chunk.embedding // Store embedding directly in the node
                        },
                        documentId
                    }
                );
        ...
        try {
            ...
            try {
                const result = await session.run(
                    ...
                    'WHERE c.documentId = $documentId OR $documentId IS NULL ' +
                    'WITH c, vector.similarity(c.embedding, $queryEmbedding) AS score ' +
                    ...
                    'RETURN c.text AS text, c.documentId AS documentId, c.chunkIndex AS chunkIndex, ' +
                ...
                // If vector similarity fails, fall back to dot product calculation
                console.warn('Vector similarity failed, falling back to dot product:', error.message);
